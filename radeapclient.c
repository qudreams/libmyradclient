...
const char *radius_dir = "/home/qudreams/raddb";
...
int rad_process_eap_request(RADIUS_PACKET* eap_rep,RADIUS_PACKET* eap_req,const char* secret,const char* pwd)
{
    ...
    for (vp = eap_req->vps; vp != NULL; vp = vpnext) {
    	...
    	case ATTRIBUTE_EAP_BASE+PW_EAP_SIM:
    		if(respond_eap_sim(eap_req,eap_rep) <= 0)
				rc = -1;
    		break;
#endif
    	}
...
static int sendrecv_eap(RADIUS_PACKET *rep)
{
	 ...
	 */
	if ((vp = pairfind(rep->vps, PW_CLEARTEXT_PASSWORD)) != NULL) {
		strncpy(password, (char *)vp->vp_strvalue, sizeof(password) - 1);
 ...
 again:
	...
	if (*password != '\0') {
		if ((vp = pairfind(rep->vps, PW_CLEARTEXT_PASSWORD)) != NULL) {
			strncpy((char *)vp->vp_strvalue, password, sizeof(password) - 1);
	...
	for (vp = req->vps; vp != NULL; vp = vpnext) {
		...
		case ATTRIBUTE_EAP_BASE+PW_EAP_MD5:
			if(respond_eap_md5(req, rep,password) && tried_eap_md5 < 3)
			{
				tried_eap_md5++;
				goto again;
			}
...
static int unmap_eapsim_types(RADIUS_PACKET *r)
{
	...
	if (esvp == NULL) {
		fprintf(stderr,"eap: EAP-Sim attribute not found");
	}
...
int main(int argc, char **argv)
{
	...
	bzero(&dst_ipaddr,sizeof(dst_ipaddr));
	if (ip_hton(argv[1], AF_INET, &dst_ipaddr) < 0) {
	...
	while(!filedone) {
        ...
        VALUE_PAIR *vp;
         ...
         //	Keep a copy of the the User-Password attribute.
        if ((vp = pairfind(req->vps, PW_CLEARTEXT_PASSWORD)) != NULL) {
            strncpy(password, (char *)vp->vp_strvalue, sizeof(password) - 1);
                                                                              
        } else 	if ((vp = pairfind(req->vps, PW_USER_PASSWORD)) != NULL) {
            strncpy(password, (char *)vp->vp_strvalue, sizeof(password) - 1);
          //	Otherwise keep a copy of the CHAP-Password attribute.
		...
		*/
		strncpy(password,"qudreams",sizeof(password));
		req = rad_create_eap_response("qudream","qudreams");
		req->dst_ipaddr = dst_ipaddr;
		req->dst_port = port;
		req->sockfd =  sockfd;

		//sendrecv_eap(req);
        rad_send_eap_response(req,secret,password);

        for(;;) {
            FD_ZERO(&fds);
            FD_SET(req->sockfd,&fds);
            nrfds = select(req->sockfd + 1,&fds,NULL,NULL,NULL);
            if(nrfds > 0) {
                rad_free(&eap_req);
                eap_req = rad_recv(req->sockfd,0);
                if(eap_req) {
                    if(rad_verify(eap_req,req,secret) == -1)
                    {
                        fprintf(stderr,"unexpted reply from server\n");
                        continue;
                    }
                    if(rad_decode(eap_req,req,secret) == -1)
                    {
                        fprintf(stderr,"decode error\n");
                        continue;
                    }

					rad_unmap_eap_types(eap_req);
                    rad_process_eap_request(req,eap_req,secret,password);
                }
