	       ...
	       const char *secret)
{
	...
	bzero(ip_buffer,sizeof(ip_buffer));
			    ...
			    const char *secret)
{
	...
	bzero(calc_digest,sizeof(calc_digest));
...
int rad_packet_ok(RADIUS_PACKET *packet, int flags)
{
	...
	bzero(host_ipaddr,sizeof(host_ipaddr));
	/*
	       ...
	       const char *secret)
{
	...
	while (packet_length > 0) {
		...
		if (vendorlen == 0) {
			...
			/*
			 *	Here,we check the unknow vendor-specific attribute to see if we can parse it or not.
			 */
			do {
				int myattr = 0;
		 ...
		 *	attribute.
		 *	now,we can parse the vendor specific attributes.
		 */
		switch (vsa_tlen) {
		case 1:
			attribute = ptr[0];
