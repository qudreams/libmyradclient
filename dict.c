			     ...
			     char **argv, int argc)
{
	...
	if (dict_addattr(argv[0], vendor, type, value, flags) < 0) {
		...
		strncpy(buffer, fr_strerror(), sizeof(buffer) - 1);
...
DICT_ATTR *dict_attrbyname(const char *name)
{
	...
	buffer = calloc(1,DICT_ATTR_MAX_NAME_LEN);
	if(buffer == NULL) {
		return NULL;
	}

	da = (DICT_ATTR *) buffer;
	strncpy(da->name, name,DICT_ATTR_MAX_NAME_LEN - sizeof(*da) - 1);

	res = fr_hash_table_finddata(attributes_byname, da);
	free(buffer);
...
DICT_VALUE *dict_valbyname(unsigned int attr, const char *name)
{
	...
	strncpy(my_dv->name, name,DICT_ATTR_MAX_NAME_LEN - sizeof(*my_dv) - 1);
...
int dict_vendorbyname(const char *name)
{
	...
	buffer = calloc(1,DICT_VENDOR_MAX_NAME_LEN);
	if(buffer == NULL) {
		return 0;
	}

	dv = (DICT_VENDOR *) buffer;
	strncpy(dv->name, name,DICT_VENDOR_MAX_NAME_LEN - sizeof(*dv) - 1);
