 ...
 * input: input block 
 * inlen: length of input block
 */

void fr_md4_calc(unsigned char* output,const unsigned char* input,unsigned int inlen)
{
	FR_MD4_CTX	context;
...
fr_MD4Final(unsigned char digest[MD4_DIGEST_LENGTH], FR_MD4_CTX *ctx)
{
	...
	*p++ = 0x80;
	...
	uint32_t* p_buf = (uint32_t*)ctx->buffer;
	p_buf[14] = ctx->count[0];
	p_buf[15] = ctx->count[1];
